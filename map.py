...
from mmengine.utils import ProgressBar, is_str
...
def bbox_map_eval(det_result, annotation, nproc=4):
    ...
    mean_aps = []
    for thr in iou_thrs:
        map_result = eval_map(bbox_det_result, [annotation], iou_thr=thr, logger='silent', nproc=1)
        mean_aps.append(map_result[0])
...
def load_ann(args):
    ...
    for a in ann['annotations']:
        ...
        cat_id = a['category_id'] - 1
        if img_id not in gt_ann:
            gt_ann[img_id] = {'bboxes': [], 'labels': []}
        gt_ann[img_id]['bboxes'].append(bbox)
        gt_ann[img_id]['labels'].append(cat_id)
    for im_id in gt_ann:
        gt_ann[im_id]['bboxes'] = torch.tensor(gt_ann[im_id]['bboxes'], dtype=torch.float32)
        gt_ann[im_id]['labels'] = np.array(gt_ann[im_id]['labels'], dtype=np.int64)
    category_nums = len(ann['categories'])
    image_names = {i['id']: i['file_name'] for i in ann['images']}
    return gt_ann, category_nums, image_names


def main(args):
    gt_ann, cat_nums, img_names = load_ann(args)
    results = load(args.pre)
    prog_bar = ProgressBar(len(results))
    _mAPs = {}

    for result in results:
        img_id = result['img_id']
        data_info = gt_ann[img_id]
        ...
        dets = []
        for label in range(cat_nums):
            index = np.where(pred_labels == label)[0]
            pred_bbox_scores = np.hstack([pred_bboxes[index], pred_scores[index].reshape((-1, 1))])
            dets.append(pred_bbox_scores)
        mAP = bbox_map_eval(dets, data_info)
        _mAPs[img_id] = mAP
        prog_bar.update()

    with open('map.txt', 'w') as f:
        for id, name in img_names.items():
            f.write(f"{name}\t{_mAPs[id]}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--pre',
        type=str,
        default='/DATA2/chenyuqing_program/mmdetection-main/my_results/TrashCan/tood/test/results.pkl'
    )
